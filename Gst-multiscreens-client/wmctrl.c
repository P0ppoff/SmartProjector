...
/* Removed for SmartProjector : Main
 * Added for SmartProjector : getWinList*/

char* getWinList()
{
    char *toReturn= malloc (2000 * sizeof (char));
    strcpy(toReturn,"");
    ...
    for (i = 0; i < client_list_size / sizeof(Window); i++) {
        ...
        sprintf(temp1,"0x%.8lx ", client_list[i]);
        sprintf(temp2," %s\n",title_out ? title_out : "N/A");

        strcat(toReturn,temp1);
